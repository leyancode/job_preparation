%==============================================================================
% CHINESE COMPARISON DRAFT. Compile: biber draft_chs; latexmk -xelatex draft_chs.tex
% draft.tex is the authoritative dissertation and evidence source. This file is an
% editorial comparison aid, not an independent claim source. When draft.tex changes,
% update the affected translation and recheck labels, citations, references, evidence
% comments and the XeLaTeX/Biber build. Captions and tabular cells intentionally remain
% in English as stable comparison anchors.
%
% 2026-08-09 RETRANSLATION. Chapters 4, 5, 6 and 7 (2D 与 3D 问题 / 性能剖析与瓶颈分析 /
%   讨论 / 结论与未来工作) were raw machine translation and have been rewritten by hand
%   against draft.tex as of that date, including the plain-language passes made to
%   sec:rqs, ch:conclusion, ch:background, sec:threats and sec:guidance.
%   Fixed throughout: the size labels 5m/10m/20m/41m/82m/165m (millions of unknowns) had
%   been rendered as 米 (metres) in ch4-ch7 -- "在32节点165米处". They are now kept in the
%   Latin form the figures and tables use. Other systematic errors removed: weak-scaling
%   comparison -> "微弱的比喻", launcher-era -> "发射装置时代", two builds -> "两座建筑",
%   saturation ordering -> "饱和命令", monotone -> "单酮".
%   Every figure, table, \label, \Cref and % VERIFIED comment is unchanged; counts checked
%   against the pre-rewrite backup (16 figures, 22 tables, 104 labels, 16 includegraphics).
%   Terminology: "stop scaling" is 停止变快 / 不再变快, not 停止扩展. 饱和 is reserved for
%   the technical sections. Chapters 1-3 were human-translated and were not touched.
%
...
比例反向上升；线程化 BLAS 最多只占求解窗口 core-time 的 2.1\%。这些测量定位了软件成本，
但没有确定底层硬件机制。
...
个节点上每核只剩约 4.9 千个未知量，已经落在 \Cref{sec:configspace} 的准入区间之外。这个有意
...
\section{MAP profiling 矩阵结果}
\label{sec:map-matrix}
两场 MAP 实验剖析的是同一个 20.25M 二维问题：一组基线跑在 \texttt{arch-omp-opt} 上，
节点数为 2、8、16；另一组与之对应，跑在生产 build \texttt{arch-omp-libsci-rome-opt} 上，节点数
为 2 和 8。两组都没有 $128\times1$ 那一格，因为 MAP 在本系统上无法每节点启动 128 个 MPI rank。
因此纯 MPI 只有 \texttt{-log\_view} 这一种证据。12 份 profile 中这里用了 11 份。16 节点的
$16\times8$ 基线 profile 因其自身内容被剔除：MAP 在其中没有解析出任何库符号，所有采样都被归给
采样器自身，这份文件不含成本信息。

一个 \texttt{.map} 文件是压缩的 XML 文档，其中的 \texttt{sample} 元素是合并调用树的
全局快照，每个节点记录了在该调用路径上处于活动状态的核心数。把树的叶子上的活动核心数加起来，
就按实际在执行的代码把整个作业的 core-time 划分开，MAP 自己的百分比就是这么算的，而且只靠文件
本身就能算。本文的分析正是这么做的，并且按文件内记录的 \texttt{srun} 命令行、而不是按文件名来
校验每份 profile 的布局，然后报告两个范围。全程序范围涵盖所有采样。求解窗口范围只保留这样的
采样：有核心处于 \texttt{KSPSolve} 内部，且没有任何核心处于 \texttt{PCSetUp} 内部，这样就仅凭
profile 近似出重复求解阶段。它是一个时间窗口，不是 PETSc 的 \texttt{RepeatedSolves} 日志阶段，
两者不可当作同一个量引用。

\Cref{tab:map-shares} 给出 11 份 profile 在求解窗口内的比例，\Cref{fig:map-shares}
将其画出。
...
两个实测比例朝相反方向移动。MPI 的比例随线程替代 rank 而下降：在 8 个节点上，
$64\times2$、$32\times4$ 和 $16\times8$ 分别是 46.7\%、22.6\% 和 6.2\%。这正是
\Cref{sec:bottleneck} 用 \texttt{-log\_view} 在整个正式矩阵上得到的次序，MAP 在事件核算层面与
之一致。OpenMP runtime 的比例则朝另一个方向走，为 48.3\%、69.3\%、76.5\%；被采样到的栈帧是
\texttt{cpu\_relax}、\texttt{do\_spin} 和 \texttt{\_\_futex\_wait}，也就是无事可做、正在等待
的线程。两个 build 在重叠处都一致：四个共同的格子里，MPI 比例相差最多 0.9 个百分点，OpenMP
比例最多 4.3 个百分点，且没有系统性的偏向。这种一致性符合布局效应，而不是 LibSci 效应。

有三条限制。MAP 的开销大且不均匀（单独测得计时阶段为 $+35\%$，\texttt{VecScatterEnd}
上约为三倍），而且它对 OpenMP 的插桩恰好扰动了正在被测量的 barrier 行为，因此这些绝对百分比
高估了未插桩运行会呈现的数值。每格只有一份 profile，所以没有观测区间。此外，由于没有
$128\times1$ 的 profile，这条趋势只在混合的那一端有锚点；不过纯 MPI 按定义就没有任何 OpenMP
runtime 时间，这把缺失的那个点固定在零。

在这些限制之内，MAP 在事件核算层面把不断增长的软件成本定位在 OpenMP 的等待路径上。
它没有指出硬件成因。固定 $16\times8$ 不变、把节点数从 2 提到 8，等待比例在 LibSci build 上从
53.5\% 升到 76.5\%，在基线 build 上从 57.8\% 升到 77.5\%。由于 rank 内部的线程放置没有变，这个
观测与"线程化工作区间变短"是一致的，但 cache 上的不均衡同样可能汇集到同一批 barrier 上。本文
测试的 rank 没有一个跨越 NUMA 边界，尽管八线程的 rank 会跨越共享 L3 的边界
（\Cref{tab:layouts}）。因此要区分剩下的这些机制，需要硬件计数器（\Cref{sec:futurework}）。

第四个观测是否定性的。线程化的 LibSci 链接进了生产 build，而 PETSc 的稀疏求解路径
几乎不调用它：在任何一份 LibSci profile 中，BLAS 最多只占求解窗口 core-time 的 2.1\%，而基线
profile 里根本没有采样到任何 BLAS 栈帧。因此线程化 BLAS 库无法解释 \Cref{ch:multinode} 测得的
布局次序，这一点此前 \Cref{sec:threats} 只能作为证据的局限来陈述，现在由 profile 数据确认。
...
这解释了 \Cref{ch:multinode} 留下的两种现象。第一种：固定三维的某一布局和某一条链，
...
把一个维度全部 33 个混合测量点合在一起会破坏这种关系，而且在两个维度上破坏的方式还
不一样。二维合并后的相关系数是 $+0.58$，而其中两种存在打平点的布局分别是 $-0.67$ 和 $-0.59$。
