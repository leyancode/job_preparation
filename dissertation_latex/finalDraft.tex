...
threaded BLAS accounts for at most 2.1\% of solve-window core-time. These measurements
...
Time per outer KSP iteration controls that convergence variation, although it does not make
GAMG hierarchy work or communication identical across decompositions. Results therefore
...
contract. At 32 nodes only about 4.9 thousand unknowns remain per core, outside the
admission window of \Cref{sec:configspace}. This deliberately over-decomposed endpoint
...
The two measured shares move in opposite directions. The MPI share falls as threads replace
ranks: at eight nodes it is 46.7\%, 22.6\% and 6.2\%
...
and MAP is consistent with it at the event-accounting level. OpenMP runtime share runs the
other way, 48.3\%, 69.3\%, 76.5\%; the sampled frames
...
by at most 4.3, with no systematic sign. This agreement is consistent with a layout effect
rather than a LibSci effect.
...
Within those limits, MAP locates the growing software cost in OpenMP wait paths at the
event-accounting level. It does not identify its hardware cause. Holding $16\times8$ fixed
from two to eight nodes raises the waiting share from 53.5\% to 76.5\% on the LibSci build
and from 57.8\% to 77.5\% on the baseline. Because thread placement within a rank is
unchanged, this observation is consistent with shorter threaded work regions, but cache
imbalance could be collected at the same barriers. No tested rank crosses a NUMA boundary,
although eight-thread ranks cross a shared-L3 boundary (\Cref{tab:layouts}). Hardware
counters are therefore needed to distinguish the remaining mechanisms
(\Cref{sec:futurework}).

A fourth observation is negative. Threaded LibSci is linked into the
...
This accounts descriptively for two patterns left by \Cref{ch:multinode}. For the first,
...
Pooling all 33 hybrid measurements of a dimension breaks the relationship, and breaks it
differently in each. In 2D the pooled correlation is $+0.58$, where the two layouts that
