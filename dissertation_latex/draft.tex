...
The synchronising events are logged \emph{inside}
...
time is not purely communication. 
...
it is strictly ordered by MPI rank count: fewer ranks, less exposure. The plotted share is
the sum of the two event classes, which sit on disjoint call paths: the halo exchange is
reached from \texttt{MatMult} and its relatives, the reductions directly from CG. The
figures plot only that sum; \Cref{tab:sync-split} separates it along one weak-scaling chain
in each dimension. Halo exchange is the larger term in every one of the 264 formal runs, by
a factor of 1.16 to 3.32 in 2D and 2.33 to 6.32 in 3D. In 2D at 165m on 32 nodes it accounts
for 34.3 of flat MPI's 48.2 percentage points, against 13.9 for the reductions. In 3D at the
same point it accounts for 70.9 of 84.6. The 3D shares are far higher than the 2D shares at
every comparable point, which is consistent with the seven-point stencil's larger
...
\begin{table}[htbp]
  ...
  two event classes, along the weak-scaling chain that ends at 165m on 32 nodes, for the
  extreme layouts. Halo is \texttt{VecScatterBegin} and \texttt{VecScatterEnd}; reduction is
  \texttt{VecTDot} and \texttt{VecNorm}. Their sum is the quantity plotted, which the
  figures already give and which is not repeated here. Each row is the repetition whose
  total is the median of its three, so both components come from the same run. Percentage
  points of \texttt{RepeatedSolves} \texttt{KSPSolve} time.}
...
%   and 2.33-6.32 in 3D. The chain shown is 5m@1, 41m@8, 82m@16, 165m@32, the one carrying
%   the 165m 32-node point quoted in the text.
...
%   draft of this table were affected. The total belongs in the figures and, for the one
%   point quoted, in the running text.
