...
\begin{table}[htbp]
  \centering
  \caption{Reduction in synchronisation exposure against time per iteration, both relative
  to flat MPI at the same size--node point. The break-even value is the exposure reduction
  the fitted line requires to offset threading overhead. Eleven points per layout; the fits
...
%   NOTE 2026-08-06: a and b are NOT columns of tab:sync-breakeven, so worked values for
%   them cannot be checked against the printed table. Illustrations that had been in the
%   text (b-1 = 43% for 16x8; |a| = 0.022, i.e. 2.2% per point, for 3D 64x2; and the
%   resulting 0.062/0.022 = 2.9) were removed for that reason -- the paragraph defines the
%   formulae and needs no instances. They are kept here, and are correct. If they are ever
%   wanted in the text, add a and b as columns first, which would also let a reader verify
%   (1-b)/a against the printed Break-even.
...
($+0.58$ in 2D, against $-0.59$ and $-0.67$ within the two layouts that have a break-even),
because $16\times8$ combines the largest exposure reduction with the worst performance. The relationship is therefore a within-layout one, and reporting a pooled
