...
one to 32 nodes, and there the layouts differ in where they stop scaling rather than only in
speed. The 2D curves stay within a factor of 1.81 of each other and all four are still improving
at 4096 cores, although the last doubling returns only 1.11 to $1.39\times$. In 3D three of
the four turn upward at a node count that rises with their thread count: eight nodes for flat
MPI, which is $2.6\times$ slower again by 32, while $16\times8$ is still improving at 32 and
ends $3.81\times$ faster than it. What
distinguishes the
...
\begin{enumerate}
  ...
  \item \textbf{Evidence that the layout ranking also depends on the scaling regime.} On
    ...
    3D layouts diverge to a factor of 3.81 because three of the four turn upward at a node
    count that rises with their thread count: eight nodes for flat MPI and 16 for the two
    intermediate layouts, with $16\times8$ still improving at 32. $16\times8$ is last at constant work
...
runtime rather than at either memory boundary, which locates the cost without settling what
...
\begin{table}[htbp]
  \centering
  \caption{Operations in one GAMG-preconditioned CG step, with the PETSc event that
  records each and the number of calls per step measured in a representative run. A step is
  one preconditioner application; PETSc's CG performs one more of these per solve than the
  iteration count it reports, because it evaluates and preconditions the initial residual
  before the first update. Neighbour and global costs scale differently with the rank
  count, which is why \Cref{sec:bottleneck} separates them.}
  \label{tab:cg-ops}
  \begin{tabular}{lllr}
    \toprule
...
The counts show why one step is dominated by neighbour exchange rather than by the
two CG dot products: the multigrid hierarchy performs its own halo exchange on every level,
so the 43 exchanges per step outnumber the three global reductions by more than an
...
removes slowly \cite{stuben2001amg}. PETSc's GAMG (geometric-algebraic multigrid,
...
The choice of preconditioner also controls experimental validity, because block Jacobi
builds one block per MPI rank and so changes the operator when the layout changes, whereas
AMG does not. \Cref{sec:solver} measures the effect and settles the comparison on GAMG.
...
reaches dense BLAS only incidentally, so the library matters here less as a source of speed
than as a possible confound: its threads would be drawn from the same per-rank thread count
and pinned to the same cores as PETSc's, and would appear in the same OpenMP runtime that
...
\begin{table}[htbp]
  ...
  conventions and are both given, because the two dimensions are quoted in both. A layout
  is counted as turning upward when its time at some node count exceeds its own minimum.
  No 2D layout does so by 32 nodes, but the last doubling still returns well under
  $2\times$ in both dimensions.}
...
\texttt{arch-omp-libsci-rome-opt} build at 2 and 8 nodes. Neither 
contains a $128\times1$
...
Within those limits the table answers the question \Cref{ch:multinode} left open, at the
level of accounting rather than of mechanism. The cost that grows with threads per rank,
and that must be repaid before hybridisation helps, is paid as idle time in the OpenMP
runtime: the term that grows is time in \texttt{cpu\_relax}, \texttt{do\_spin} and
\texttt{\_\_futex\_wait}, not time in the compute kernels. NUMA placement is excluded on
topology alone: a rank of eight threads crosses a shared-L3 boundary but still fits inside
one 16-core NUMA region, so no layout of this study places a rank across two NUMA regions
and the four cannot differ in that respect (\Cref{tab:layouts}). 

Neither is the growth a placement effect, because it is present at fixed layout, 
where a rank's threads occupy the same cores against the same shared-L3 boundary at every node count: 
holding $16\times8$ and going from two nodes to eight raises the waiting share from 53.5\% to
76.5\% on the LibSci build and from 57.8\% to 77.5\% on the baseline, as the work per
threaded region shrinks. 

That is the discriminating observation. The other two features of
the shape, monotonicity in the thread count and a maximum exactly where $16\times8$
performs worst, are consistent with this reading but equally consistent with a cache one.
What the profiles do not settle is why the threads wait: a rank spanning two CCXs could
produce imbalance that is collected at the same barrier, and \Cref{sec:sync-limits} leaves
that to the hardware-counter work of \Cref{sec:futurework}. \Cref{sec:bottleneck} shows
...
table, which is consistent with the seven-point stencil's larger surface-to-volume ratio.
...
what a layout would cost against flat MPI if it removed no exposure at all, so $b-1$ is the
cost of threading alone. The slope $a$ is the change in $\mathrm{TimeRatio}$ per point of
exposure removed, so $|a|$ is how much of that cost one point pays back. Since $a<0$, the
...
Read down $b$, the cost of threading before any exposure is removed: it rises sharply at
each doubling, and at $32\times4$ it is almost independent of the dimension, 1.1305 against
...
point of removed exposure: it falls at every step, and at all three layouts it is larger in
3D than in 2D. 
...
along the horizontal axis, each removing more exposure than the one before it and each
needing more of it before crossing parity; the bands flatten as threads per rank rise, which
...
This explains the two things \Cref{ch:multinode} could not. The first is why the 3D
advantage rises and falls with node count. Holding a layout and a chain fixed and walking the
node count, $\mathrm{ExposureRemoved}$ and $\mathrm{TimeRatio}$ reverse together at every
...
advantage is non-monotone because $\mathrm{ExposureRemoved}$ is, not because the mechanism
differs. That is a statement about the difference and not about either share: along this
...
a direct observation and needs no fit. Some other cost dominates. \Cref{tab:map-shares}
identifies that cost: at the same problem size and node counts, $16\times8$ spends more of its core-time
...
Pooling all 33 hybrid measurements of a dimension destroys the relationship, and does so
differently in each. In 2D the pooled correlation is $+0.58$, against $-0.67$ and $-0.59$
within the two layouts that have a break-even; in 3D it collapses to $+0.10$ from three
values that all exceed $0.8$ in magnitude. The obvious explanation, that $16\times8$
combines the largest exposure reduction with the worst performance, is sufficient in 3D,
where dropping that layout restores a pooled $-0.75$, but not in 2D, where dropping it still
leaves $+0.13$: $32\times4$ there already removes more exposure than $64\times2$ over its
whole range while being slower over its whole range. The between-layout ordering opposes the
within-layout one in both dimensions and $16\times8$ is only its most extreme case. The
relationship is therefore a within-layout one, and reporting a pooled figure would be
misleading.

The weak-scaling chains cannot be pooled either, and that is a second and independent
effect. In 3D pooling them is benign: every layout's per-chain fits bracket its pooled one
and the three break-evens move by at most 2.4 points. In 2D none of the three survives
pooling unchanged, and $16\times8$ inverts outright, as \Cref{sec:sync-breakeven} sets out.
\Cref{tab:sync-breakeven} nevertheless reports pooled fits, because five and six points are
too few to fit a chain separately, and its columns should be read with that in mind: the
cross-dimension agreement of the $b$ column is a property of the pooled fits, loosening at
$32\times4$ when the chains are taken apart and failing on the $\sim$40k chain at
$16\times8$. Neither pooling changes which layouts beat flat MPI, because that is read off
the measurements and not off a fit.
...
to 32 nodes and all are still scaling at the end, on a last doubling worth 1.11 to
$1.39\times$. On the matched 20.35M 3D problem they
diverge: three of the four turn upward, and the node count at which they do rises with the
...
PETSc builds. This locates the cost that hybridisation must repay in the OpenMP runtime
rather than at a memory boundary, and it explains why $16\times8$ removes the most exposure
in 2D and is still the slowest layout at every point. Where the cost is paid is not why it
is paid; \Cref{sec:rq4} takes that up.
...
other and all four are still improving at 4096 cores, on a last doubling worth 1.11 to
$1.39\times$. The matched 3D curves diverge, because three of the four turn upward at a node
count that rises with their thread count: eight nodes for flat MPI, 16 for $64\times2$ and
